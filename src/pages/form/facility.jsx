...
const FacilityForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('https://idsr-backend.onrender.com/api/facility/', payload);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto p-6 bg-white rounded shadow space-y-6">
      ...
      <Section title="Facility Details" sectionKey="facility">
        {renderInput('patient_id', 'Patient ID')}   
        {renderSelect('designation', 'Designation', formOptions.designation)}
        {renderSelect('form_completed_by', 'Form Completed By', formOptions.form_completed_by)}
        ...
        {renderSelect('reporting_method', 'Reporting Method', formOptions.reporting_method)}
      </Section>
